...
MERGE is a file-copy bucket alongside `overwrite_files`, `noexist_files`, `devel_files`,
`test_files`, `header_files`, and `requirements_files`. Today it is implemented by a single
helper, `repolib.files.merge_at_imports_safe`, that performs set-union merge on `@`-import lines.
