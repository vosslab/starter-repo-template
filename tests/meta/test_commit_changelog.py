...
def test_compute_new_entries_empty_prior_returns_all() -> None:
...
def test_compute_new_entries_same_day_second_commit() -> None:
...
def test_compute_new_entries_no_changes_returns_empty() -> None:
	entries = [make_entry("2026-05-21", "a")]
	out = commit_changelog.compute_new_entries(entries, entries)
	assert out == []

def test_compute_new_entries_title_rephrase_treated_as_new() -> None:
...
def test_compute_new_entries_preserves_current_order() -> None:
...
def test_parse_text_round_trip_drives_compute_new_entries() -> None:
