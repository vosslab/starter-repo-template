...
def test_propagate_spec_all_entries_resolve_to_source():
	...
	# Import the propagator module
	sys.path.insert(0, repo_root)
	import propagate_style_guides as pg

	for repo_type in ('python', 'typescript', 'rust', 'other'):
		plan = pg.compute_propagation_plan(repo_root, repo_type)
		for bucket, entries in plan.items():
			if bucket == 'gitignore_block':
				continue
			for entry in entries:
				source_path = pg.source_path_for_bucket(repo_root, bucket, entry, repo_type=repo_type)
